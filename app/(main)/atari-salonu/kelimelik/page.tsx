...
import KelimelikOyun from "@/components/oyun/KelimelikOyun";
import { Trophy, Type, ArrowLeft } from "lucide-react";
import Link from "next/link";

type ScoreEntry = { username: string; avatarUrl: string | null; score: number; updatedAt: string };

export default function KelimelikSayfa() {
  ...
  const fetchScores = useCallback(async () => {
    try {
      const res = await fetch("/api/oyun/skor?game=kelimelik");
    ...
    if (score > 0) {
      try {
        const res = await fetch("/api/oyun/skor", {
          ...
          body: JSON.stringify({ score, game: "kelimelik" }),
        });
        const data = await res.json();
  ...
  return (
    <div className="w-full px-4 lg:px-8 py-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-lg font-bold">kelimelik</h1>
          <p className="text-xs text-muted-foreground">5 harfli türkçe kelimeyi 6 denemede bul. wordle türkçe.</p>
        </div>
      </div>
      <div className="mb-6">
        <KelimelikOyun onGameOver={handleGameOver} />
