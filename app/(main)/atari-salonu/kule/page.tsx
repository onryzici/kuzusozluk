...
import KuleUstasiOyun from "@/components/oyun/KuleUstasiOyun";
...
export default function KuleSayfa() {
  ...
  const fetchScores = useCallback(async () => {
    try {
      const res = await fetch("/api/oyun/skor?game=kule");
    ...
    if (score > 0) {
      try {
        const res = await fetch("/api/oyun/skor", {
          ...
          body: JSON.stringify({ score, game: "kule" }),
        });
        const data = await res.json();
  ...
  return (
    <div className="w-full px-4 lg:px-8 py-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-lg font-bold">kule ustası</h1>
          <p className="text-xs text-muted-foreground">blokları tam üst üste bindir, dışarda kalan kesilir.</p>
        </div>
      </div>
      <div className="mb-6">
        <KuleUstasiOyun onGameOver={handleGameOver} />
      ...
      <div className="border border-border rounded-lg overflow-hidden">
        <div className="flex items-center gap-1.5 px-4 py-2.5 border-b border-border bg-primary/5">
          ...
          <span className="text-xs font-medium text-primary">en yüksek kuleler</span>
        </div>
