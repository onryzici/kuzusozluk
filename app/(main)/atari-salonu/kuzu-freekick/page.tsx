...
import KuzuFreekickOyun from "@/components/oyun/KuzuFreekickOyun";
...
export default function KuzuFreekickSayfa() {
  ...
  const fetchScores = useCallback(async () => {
    try {
      const res = await fetch("/api/oyun/skor?game=kuzu-freekick");
      ...
      if (score > 0) {
        try {
          const res = await fetch("/api/oyun/skor", {
            ...
            body: JSON.stringify({ score, game: "kuzu-freekick" }),
          });
          const data = await res.json();
  ...
  return (
    <div className="w-full px-4 lg:px-8 py-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-lg font-bold">kuzu freekick</h1>
          <p className="text-xs text-muted-foreground">
            3 aşamalı vuruş: nişan, güç, falso. duvardan ve kaleciden geçen gol olur. zorluk gittikçe artar.
          </p>
        </div>
      </div>

      <div className="mb-6">
        <KuzuFreekickOyun onGameOver={handleGameOver} />
