...
import LeninistOyun from "@/components/oyun/LeninistOyun";
...
export default function LeninistSayfa() {
  ...
  const fetchScores = useCallback(async () => {
    try {
      const res = await fetch("/api/oyun/skor?game=leninist-sapla");
      ...
      if (score > 0) {
        try {
          const res = await fetch("/api/oyun/skor", {
            ...
            body: JSON.stringify({ score, game: "leninist-sapla" }),
          });
          const data = await res.json();
  ...
  return (
    <div className="w-full px-4 lg:px-8 py-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-lg font-bold">leninist&apos;e sapla</h1>
          <p className="text-xs text-muted-foreground">
            dönen tahtaya bıçak fırlat. işaretli hedef bölgelere isabet bonus puan, saplı bıçaklara çarparsan biter.
          </p>
        </div>
      </div>

      <div className="mb-6">
        <LeninistOyun onGameOver={handleGameOver} />
