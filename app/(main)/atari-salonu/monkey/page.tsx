...
import MonkeyOyun from "@/components/oyun/MonkeyOyun";
...
export default function MonkeySayfa() {
  ...
  const fetchScores = useCallback(async () => {
    try {
      const res = await fetch("/api/oyun/skor?game=monkey");
    ...
    if (score > 0) {
      try {
        const res = await fetch("/api/oyun/skor", {
          ...
          body: JSON.stringify({ score, game: "monkey" }),
        });
        const data = await res.json();
  ...
  return (
    <div className="w-full px-4 lg:px-8 py-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-lg font-bold">spank the monkey</h1>
          <p className="text-xs text-muted-foreground">
            eli surukleyip maymuna vur. ne kadar hizli vurursan o kadar uzaga gider!
          </p>
        </div>
      </div>

      <div className="mb-6">
        <MonkeyOyun onGameOver={handleGameOver} />
        {lastScore !== null && lastScore > 0 && (
          <div className="text-center mt-2 text-sm">
            son skor: <span className="font-bold text-primary">{lastScore} km/s</span>
          ...
          {scores.length === 0 ? (
            ...
            scores.map((entry, idx) => (
              <div key={entry.username} className="flex items-center gap-3 px-4 py-2.5 hover:bg-accent/50 transition-colors">
                ...
                <span className="text-sm font-mono font-bold text-primary">{entry.score.toString().padStart(5, "0")} km/s</span>
              </div>
            ))
          )}
        </div>
