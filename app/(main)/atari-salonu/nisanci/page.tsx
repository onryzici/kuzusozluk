...
import KuzuNisanciOyun from "@/components/oyun/KuzuNisanciOyun";
...
export default function NisanciSayfa() {
  ...
  const fetchScores = useCallback(async () => {
    try {
      const res = await fetch("/api/oyun/skor?game=nisanci");
      ...
      if (score > 0) {
        try {
          const res = await fetch("/api/oyun/skor", {
            ...
            body: JSON.stringify({ score, game: "nisanci" }),
          });
          const data = await res.json();
  ...
  return (
    <div className="w-full px-4 lg:px-8 py-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-lg font-bold">kuzu nişancı</h1>
          <p className="text-xs text-muted-foreground">60 saniye, kuzulara tıkla, bombalardan kaç. zorluk gittikçe artar.</p>
        </div>
      </div>

      <div className="mb-6">
        <KuzuNisanciOyun onGameOver={handleGameOver} />
