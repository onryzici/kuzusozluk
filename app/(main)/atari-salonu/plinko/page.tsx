...
import PlinkoOyun from "@/components/oyun/PlinkoOyun";
...
export default function PlinkoSayfa() {
  ...
  const fetchScores = useCallback(async () => {
    try {
      const res = await fetch("/api/oyun/skor?game=plinko");
    ...
    if (score > 0) {
      try {
        const res = await fetch("/api/oyun/skor", {
          ...
          body: JSON.stringify({ score, game: "plinko" }),
        });
        const data = await res.json();
  ...
  return (
    <div className="w-full px-4 lg:px-8 py-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-lg font-bold">plinko</h1>
          <p className="text-xs text-muted-foreground">10 top, çivili tahta. uçtaki slotlar mega çarpan, ortadakiler az.</p>
        </div>
      </div>
      <div className="mb-6">
        <PlinkoOyun onGameOver={handleGameOver} />
