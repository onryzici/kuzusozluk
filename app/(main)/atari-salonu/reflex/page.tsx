...
import ReflexTestiOyun from "@/components/oyun/ReflexTestiOyun";
...
export default function ReflexSayfa() {
  ...
  const fetchScores = useCallback(async () => {
    try {
      const res = await fetch("/api/oyun/skor?game=reflex");
      ...
      if (score > 0) {
        try {
          const res = await fetch("/api/oyun/skor", {
            ...
            body: JSON.stringify({ score, game: "reflex" }),
          });
          const data = await res.json();
  ...
  return (
    <div className="w-full px-4 lg:px-8 py-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-lg font-bold">reflex testi</h1>
          <p className="text-xs text-muted-foreground">5 turluk reaksiyon ölçümü. ne kadar hızlıysan o kadar puan.</p>
        </div>
      </div>

      <div className="mb-6">
        <ReflexTestiOyun onGameOver={handleGameOver} />
      ...
      <div className="border border-border rounded-lg overflow-hidden">
        <div className="flex items-center gap-1.5 px-4 py-2.5 border-b border-border bg-primary/5">
          ...
          <span className="text-xs font-medium text-primary">en hızlı reflexler</span>
        </div>
