...
import SolucanOyun from "@/components/oyun/SolucanOyun";
...
export default function SolucanSayfa() {
  ...
  const fetchScores = useCallback(async () => {
    try {
      const res = await fetch("/api/oyun/skor?game=solucan");
    ...
    if (score > 0) {
      try {
        const res = await fetch("/api/oyun/skor", {
          ...
          body: JSON.stringify({ score, game: "solucan" }),
        });
        const data = await res.json();
  ...
  return (
    <div className="w-full px-4 lg:px-8 py-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-lg font-bold">yilan seven solucan</h1>
          <p className="text-xs text-muted-foreground">
            yemekleri topla, buyumeye devam et. duvara veya kendine carpma!
          </p>
        </div>
      </div>

      <div className="mb-6">
        <SolucanOyun onGameOver={handleGameOver} />
