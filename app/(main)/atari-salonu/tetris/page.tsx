...
import TetrisOyun from "@/components/oyun/TetrisOyun";
...
export default function TetrisSayfa() {
  ...
  const fetchScores = useCallback(async () => {
    try {
      const res = await fetch("/api/oyun/skor?game=tetris");
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            satirlari tamamla, seviye atla. klasik tetris deneyimi.
          </p>
        </div>
      </div>

      <div className="mb-6">
        <TetrisOyun onGameOver={handleGameOver} />
          ...
          {scores.length === 0 ? (
            ...
            scores.map((entry, idx) => (
              <div key={entry.username} className="flex items-center gap-3 px-4 py-2.5 hover:bg-accent/50 transition-colors">
                ...
                <span className="text-sm font-mono font-bold text-primary">{entry.score.toString().padStart(6, "0")}</span>
              </div>
            ))
          )}
        </div>
