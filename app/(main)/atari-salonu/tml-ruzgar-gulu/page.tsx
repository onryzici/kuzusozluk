...
import TmlRuzgarGuluOyun from "@/components/oyun/TmlRuzgarGuluOyun";
import { Trophy, Wind, ArrowLeft } from "lucide-react";
import Link from "next/link";

type ScoreEntry = { username: string; avatarUrl: string | null; score: number; updatedAt: string };

export default function TmlRuzgarGuluSayfa() {
  ...
  const fetchScores = useCallback(async () => {
    try {
      const res = await fetch("/api/oyun/skor?game=tml-ruzgar-gulu");
      ...
      if (score > 0) {
        try {
          const res = await fetch("/api/oyun/skor", {
            ...
            body: JSON.stringify({ score, game: "tml-ruzgar-gulu" }),
          });
          const data = await res.json();
  ...
  return (
    <div className="w-full px-4 lg:px-8 py-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-lg font-bold">tml rüzgar gülü</h1>
          <p className="text-xs text-muted-foreground">
            rüzgar gülünü tut, çevir, bırak. ne kadar hızlı çevirirsen o kadar puan.
            tekrar çekersen combo başlar. süre 60 saniye.
          </p>
        </div>
      </div>

      <div className="mb-6">
        <TmlRuzgarGuluOyun onGameOver={handleGameOver} />
      ...
      <div className="border border-border rounded-lg overflow-hidden">
        <div className="flex items-center gap-1.5 px-4 py-2.5 border-b border-border bg-primary/5">
          ...
          <span className="text-xs font-medium text-primary">en yüksek skorlar</span>
        </div>
